...
<%          desc = desc[76:] %>\
    ...
    %for field in sorted(obj._pt_values(), key=lambda x: x._pt_lsb):
        ...
        self.${field._pt_name} = ${field._pt_name | tc.snake_case}
        %elif type(field._pt_container) in (Enum, Struct):
        self.${field._pt_name | tc.snake_case} = ${field._pt_container._pt_name}(${field._pt_name | tc.snake_case})
        %endif
    %endfor
        # If a value was provided, decode it
        if _value != None: self.unpack(_value)
    ...
    %for field in sorted(obj._pt_values(), key=lambda x: x._pt_lsb):
        %if isinstance(field, Scalar):
        scalar |= (self.${field._pt_name} & 0x${f"{field._pt_mask:X}"}) << ${field._pt_lsb}
        %elif isinstance(field._pt_container, Struct):
        scalar |= (self.${field._pt_name}.pack() & 0x${f"{field._pt_mask:X}"}) << ${field._pt_lsb}
        %elif isinstance(field._pt_container, Enum):
        scalar |= (int(self.${field._pt_name}) & 0x${f"{field._pt_mask:X}"}) << ${field._pt_lsb}
        %endif
    %endfor
        return scalar
    ...
    %for field in sorted(obj._pt_values(), key=lambda x: x._pt_lsb):
        %if isinstance(field, Scalar):
        self.${field._pt_name} = (scalar >> ${field._pt_lsb}) & 0x${f"{field._pt_mask:X}"}
        %elif isinstance(field._pt_container, Struct):
        self.${field._pt_name}.unpack((scalar >> ${field._pt_lsb}) & 0x${f"{field._pt_mask:X}"})
        %elif isinstance(field._pt_container, Enum):
        self.${field._pt_name} = ${field._pt_container._pt_name}((scalar >> ${field._pt_lsb}) & 0x${f"{field._pt_mask:X}"})
        %endif
    %endfor
